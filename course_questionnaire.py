...
def course_questionnaire(driver):
    ...
    try:
        ...
        WebDriverWait(driver, 20).until(
            EC.element_to_be_clickable((By.XPATH,  "//div[@class='mat-tab-label-content'][contains(text(),'問卷')]"))
        ).click()
        time.sleep(2)
        if "問卷一" in driver.page_source:
            print("\033[32m進入問卷成功\033[0m")
        else:
            print("\033[31m進入問卷失敗\033[0m")

        # 繳交問卷
        time.sleep(2)
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//p[@class='qtn__title' and text()='問卷一']"))
        ).click()
        time.sleep(2)
        if "問卷名稱" in driver.page_source and "作答說明" in driver.page_source:
            print("\033[32m觀看問卷說明成功\033[0m")
        else:
            print("\033[31m觀看問卷說明失敗\033[0m")   
        ...
        if "繳交成功" in driver.page_source:
            print("\033[32m繳交問卷成功\033[0m")
        else:
            print("\033[31m繳交問卷失敗\033[0m") 
        time.sleep(2)      
        WebDriverWait(driver, 10).until(
