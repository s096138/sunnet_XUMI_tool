...
def teacher_hw_questionbank(driver):
    try:
        print("測試：辦公室-作業管理-題庫維護")
        menu_expanded_with_sibling(driver, "作業管理", "題庫維護")
        time.sleep(2)
        ...
        driver.execute_script("do_search_item()")
        time.sleep(2)
        if "查無相關資料" in driver.page_source:
            print("\033[32m勾選難易度搜尋正確\033[0m")
        else:
            td1 = driver.find_element(By.XPATH, "//*[@id='mainTable']/tbody/tr[1]/td[10]").text
            time.sleep(2)
            if td1 == "適中":
                print("\033[32m勾選難易度搜尋正確\033[0m")
            else:
        ...
        # 複製
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "(//input[@name='sel[]'])[1]"))
        ).click()  
