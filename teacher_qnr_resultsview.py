...
def teacher_qnr_resultsview(driver):
    try:
        print("測試：辦公室-問卷管理-結果檢視")
        menu_expanded(driver, "問卷管理", "結果檢視")
        time.sleep(2)
        if "問卷一" in driver.page_source and "檢視" in driver.page_source:
        ...
        WebDriverWait(driver, 20).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(text(),'檢視')]"))
        ).click()        
        time.sleep(2)
        ...
        time.sleep(2)
        if "序號" in driver.page_source and "夾檔" in driver.page_source:
